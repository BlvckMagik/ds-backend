    ...
    this.bot.command('start', async (ctx) => {
      ...
      try {
        ...
        const response = await this.chatService.getChatResponse(
          ...
          Заняття тільки онлайн в Google meets, Zoom, Discord
          Якщо користувач попросить посилання на соц мережі: Instagram: https://www.instagram.com/drako_schule
          Якщо користувач запитає чи можна займатись з домашніми улюбленцями, то повідом, що якщо користувач буде почувати себе більше комфортно на уроці, то це тільки вітається.
          ...
          НАДАВАЙ ВІДПОВІДІ БАЗУЮЧИСЬ НА ЦІЙ ІНФОРМАЦІЇ, ЯКЩО ІНФОРМАЦІЇ ПО ПИТАННЮ КОРИСТУВАЧА НЕМАЄ, проси звернутись до менеджера школи. Дай посилання на його телеграм: @drako_schule!!!
          ...
          Один раз під час консультації запропонуй сформувати заявку на навчання. Якщо користувач відмовляється, то продовжуй консультацію і запропонуй сформувати заявку тільки після того, як всі питання вирішені.
